...
class Game:
    ...
    def run(self):
        ...
        while not self._game_over():
            ...
            row, col = move
            if not self._first_player_moves and current_player == self._playerX:
                self._first_player_moves.append((row, col))
        ...
        write_to_csv(winner, len(self._first_player_moves), self._first_player_moves)
    ...
    def get_winner(self):
        ...
        for a in range(len(self._board.get_board())):
            if self._board.get_board()[0][a] == self._board.get_board()[1][a] == self._board.get_board()[2][a] and self._board.get_board()[0][a] is not None:
                return self._board.get_board()[0][a]

        if all(self._board.get_board()[i][i] == self._board.get_board()[0][0] and self._board.get_board()[i][i] is not None for i in range(3)) or all(
                self._board.get_board()[i][2 - i] == self._board.get_board()[0][2] and self._board.get_board()[i][2 - i] is not None for i in range(3)):
            return self._board.get_board()[1][1]

        if all(cell is not None for row in self._board.get_board() for cell in row):
            return None

        return None

    def _game_over(self):
        return self.get_winner() or all(
